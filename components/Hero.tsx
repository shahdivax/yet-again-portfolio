        ...
        <section className="w-full grid grid-cols-1 xl:grid-cols-[1fr_minmax(auto,1000px)_1fr] border-b border-[var(--border)] relative z-10 bg-transparent pt-10">
            ...
            <div className="w-full grid grid-cols-12 gap-0 border-x xl:border-x-0 border-[var(--border)] bg-[var(--background)]">
                ...
                >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {profileLinks.map((link) => (
                            <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className={`w-full px-3 py-3 text-[9px] font-mono font-bold tracking-[0.16em] uppercase transition-all duration-300 border border-[var(--border)] flex items-center justify-center gap-2 ${isDark ? "hover:bg-white hover:text-black" : "hover:bg-black hover:text-white"}`}
                            >
                                <img
                                    src={link.logo}
                                    alt=""
                    ...
                    <div className={`border border-[var(--border)] p-2.5 ${isDark ? "bg-white/[0.02]" : "bg-black/[0.02]"}`}>
                        <div className={`mb-2 font-mono text-[7px] tracking-[0.22em] uppercase ${isDark ? "text-zinc-500" : "text-zinc-500"}`}>
                            Open in
                        </div>
                        <div className="grid grid-cols-3 gap-2">
                            {aiCopyTargets.map((target) => (
                                <a
                                    key={target.label}
                                    href={`${target.href}${encodeURIComponent(aiContextPrompt)}`}
                                    target="_blank"
                                    rel="noreferrer"
                                    aria-label={target.label}
                                    title={target.label}
                                    className={`group/ai flex h-10 items-center justify-center border border-[var(--border)] transition-all duration-300 ${isDark ? "bg-black/20 hover:bg-white hover:border-white" : "bg-white hover:bg-black hover:border-black"}`}
                                >
                                    <img
                                        src={target.logo}
                                        alt=""
                                        aria-hidden="true"
                                        loading="lazy"
                                        className="h-5 w-5 object-contain transition-transform duration-300 group-hover/ai:scale-110"
                                    />
                                </a>
                            ))}
                        </div>
